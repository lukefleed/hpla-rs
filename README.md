...
* **`eigen/csc_map`**: C++ FFI bindings to [Eigen 3](https://eigen.tuxfamily.org/). Uses an `Eigen::Map` to project the CSC memory buffer into an `Eigen::SparseMatrix` without copies. The multiplication is evaluated purely via C++ Template Expressions (`*y += (*A) * (*x);`).
