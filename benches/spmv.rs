...
use spmv_bench::petsc::{libpetsc_spmv_setup, libpetsc_spmv_execute, libpetsc_spmv_teardown};
...
fn bench_spmv(c: &mut Criterion) {
    ...
    for path in matrices {
        ...
        let bytes = ((raw.nrows + 1) * 4 + raw.nnz * 4 + raw.nnz * 8 + raw.ncols * 8 + raw.nrows * 8) as u64;
        group.throughput(Throughput::Bytes(bytes));

        // ----------------------------------------------------
        // Faer
        // ----------------------------------------------------
        let a_faer = SparseColMat::try_new_from_triplets(raw.nrows, raw.ncols, &raw.triplets).unwrap();
