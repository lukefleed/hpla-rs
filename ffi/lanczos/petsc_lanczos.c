// PETSc FFI wrapper for one-pass Lanczos computing exp(-A)b.
// Stores the full Krylov basis V_k (n x k, column-major, O(nk) memory).
// Final reconstruction is a single dense GEMV: x = ||b|| * V_k * g.
//
// Rolling buffer rotation uses VecPlaceArray (O(1) pointer swap) instead
// of VecSwap (O(n) BLASswap). The three Vec shells are created once via
// VecCreateSeqWithArray and repointed each step.
...
PetscLanczosCtx* libpetsc_lanczos_setup(
    int32_t nrows,
    int32_t ncols,
    int32_t nnz,
    ...
    int32_t        krylov_dim)
{
    ...
    PetscLanczosCtx *ctx = malloc(sizeof *ctx);
    if (!ctx) return NULL;
    ...
    ctx->V_k      = malloc((size_t)nrows * krylov_dim * sizeof(double));
...
void libpetsc_lanczos_execute(PetscLanczosCtx *ctx) {
    ...
    // Store V_k column 0.
    memcpy(ctx->V_k, ctx->raw_curr, n * sizeof(double));
    ...
    for (int32_t i = 0; i < k; i++) {
        ...
        if (beta <= breakdown_tol) break;

        if (i < k - 1) {
            ctx->betas[i] = beta;

            // Normalize: work *= 1/beta.
            VecScale(ctx->work, 1.0 / beta);

            // O(1) pointer rotation.
            ...
            // Store V_k column i+1.
            memcpy(ctx->V_k + (size_t)(i + 1) * n, ctx->raw_curr,
                   n * sizeof(double));
        }
    }

    // exp(-T_m) * e_1 via eigendecomposition of -T_m.
    ...
    // g = Q * weights = exp(-T_m) * e_1, via BLAS dgemv.
    {
        PetscBLASInt one_i = 1;
        double one_d = 1.0, zero_d = 0.0;
        BLASgemv_("N", &bm, &bm, &one_d, z, &bm,
                  weights, &one_i, &zero_d, g, &one_i);
    }

    // x = b_norm * V_k(:, 0:m) * g via BLAS dgemv.
    {
        PetscBLASInt bn = (PetscBLASInt)n;
        PetscBLASInt bm2 = (PetscBLASInt)m;
        PetscBLASInt one_i = 1;
        double zero_d = 0.0;

        PetscScalar *x_array;
        VecGetArray(ctx->x, &x_array);
        BLASgemv_("N", &bn, &bm2, &b_norm, ctx->V_k, &bn,
                  g, &one_i, &zero_d, x_array, &one_i);
        VecRestoreArray(ctx->x, &x_array);
    }
}

void libpetsc_lanczos_get_y(PetscLanczosCtx *ctx, double *restrict out, int32_t len) {
...
void libpetsc_lanczos_teardown(PetscLanczosCtx *ctx) {
    if (!ctx) return;
    MatDestroy(&ctx->A);
